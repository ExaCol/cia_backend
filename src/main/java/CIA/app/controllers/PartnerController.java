...
public class PartnerController {
    ...
    public ResponseEntity<?> getByUser(@RequestHeader("Authorization") String authHeader, @PathVariable Integer serviceId) {
        String token = authHeader.replace("Bearer ", "");
        try{
            String email = jwtUtil.extractEmail(token);
            String role = jwtUtil.extractUserRole(token);
            ...
            if (jwtUtil.isTokenValid(token, email) && "Cliente".equals(role) ) {
                try {
                    List<Partner> p = partnerService.getPartnersByServices(serviceId);
                    if (p == null || p.isEmpty()) {
