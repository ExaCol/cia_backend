...
public class ServicesController {
    ...
    @GetMapping("/specificServices")
    public ResponseEntity<?> getService(@RequestHeader("Authorization") String authHeader, @RequestBody Services services) {
        String token = authHeader.replace("Bearer ", "");
        String email = jwtUtil.extractEmail(token);
        String role = jwtUtil.extractUserRole(token);
        ...
        if (jwtUtil.isTokenValid(token, email) && "Cliente".equals(role) ) {
            try {
                Services s = servicesService.getSpecificServices(services);
                if (s == null) {
    ...
    public ResponseEntity<?> deleteSpecificService(@RequestHeader("Authorization") String authHeader, @RequestBody Services services) {
        String token = authHeader.replace("Bearer ", "");
        String email = jwtUtil.extractEmail(token);
        String role = jwtUtil.extractUserRole(token);
